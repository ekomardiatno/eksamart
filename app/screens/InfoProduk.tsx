...
interface Props {
  navigation: NativeStackNavigationProp<RootStackParamList, 'InfoProduk'>;
  route: RouteProp<RootStackParamList, 'InfoProduk'>;
  insets: EdgeInsets;
}
...
class InfoProduk extends Component<Props, State> {
  ...
  componentDidMount() {
    ...
    AsyncStorage.getItem('produk').then(item => {
      console.log(item)
      if (item !== null) {
        const parsed: Produk[] = JSON.parse(item)
        this.setState({
          data: parsed
        }, () => {
          this._check()
  ...
  closeAlert = (finisher: (() => void) | null = null) => {
    this.fadeOut()
    this.toBottom()
    setTimeout(() => {
      this.setState({
  ...
  simpan = async () => {
    ...
    let produk = {
      'id_produk': id,
      'nama_produk': nama,
      'modal_produk': parseInt(String(modal)),
      'harga_produk': parseInt(String(harga)),
      'diskon_produk': parseInt(String(diskon)),
      'stok_produk': 0
    }

    const rs = await AsyncStorage.getItem('produk')
    if (rs !== null) {
      let data: any[] = JSON.parse(rs)
      let data2: any[] = JSON.parse(rs)
      data2.push(produk)
      let dataBaru = data2
      this.setState({
      ...
      data.forEach((item: any, index: number) => {
        if (item.id_produk == this.props.route.params.id) {
          ...
          data[index].harga_produk = parseInt(String(harga))
          data[index].diskon_produk = parseInt(String(diskon))
          data[index].modal_produk = parseInt(String(modal))
          this.setState({
...
export default withSafeAreaInsets<Omit<Props, 'insets'>>(InfoProduk)
