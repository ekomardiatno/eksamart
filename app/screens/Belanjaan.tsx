...
interface Props {
  navigation: NativeStackNavigationProp<RootStackParamList, 'Belanjaan'>;
  route: RouteProp<RootStackParamList, 'Belanjaan'>;
  insets: EdgeInsets;
}
...
class Belanjaan extends Component<Props, State> {
  ...
  componentDidMount() {
    this.backHandler = BackHandler.addEventListener('hardwareBackPress', this._backHandler as any)
    this._getData()
    AsyncStorage.getItem('penjualan').then(item => {
      this.setState({
        belanjaan: JSON.parse(item as string)
      })
    })
    AsyncStorage.getItem('produk').then(p => {
      this.setState({
        produk: JSON.parse(p as string)
      })
    })
  }
...
export default withSafeAreaInsets<Omit<Props, 'insets'>>(Belanjaan)
