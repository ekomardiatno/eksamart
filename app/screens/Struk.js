...
class Struk extends Component {
  ...
  _mkdir = (finisher = null) => {
    RNFS.exists(this.state.path).then(status => {
      if (!status) {
        RNFS.mkdir(this.state.path).then(res => {
    ...
    this.setState(
      {
        transaksi:
          transaksi !== null
            ? JSON.parse(transaksi).filter(t => {
                return t.id_transaksi === this.props.route.params.id_transaksi;
              })[0]
            : null,
        penjualan:
          penjualan !== null
            ? JSON.parse(penjualan).filter(t => {
                return t.id_transaksi === this.props.route.params.id_transaksi;
              })
            : [],
  ...
  _captureView = () => {
    this._viewShot.current.capture().then(uri => {
