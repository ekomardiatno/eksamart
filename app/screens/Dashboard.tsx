...
import { Produk, Penjualan, Transaksi } from '../types'
...
class Dashboard extends Component<Props, State> {
  ...
  _getData = async (finisher: (() => void) | null = null) => {
    ...
    if (data != null) {
      let dateObj = new Date(),
        ...
        m = (dateObj.getMonth() + 1).toString().length < 2 ? '0' + (dateObj.getMonth() + 1) : (dateObj.getMonth() + 1),
        d = dateObj.getDate().toString().length < 2 ? '0' + dateObj.getDate() : dateObj.getDate(),
        push: any[] = [],
        count = 0
  ...
  _getPenjualan = async (finisher: (() => void) | null = null) => {
  ...
  _getProduk = async (finisher: (() => void) | null = null) => {
  ...
  _getTotal = () => {
    let date = new Date(),
      y = date.getFullYear(),
      m = parseInt(String(date.getMonth())) + 1,
      d = date.getDate()
  ...
  _keyExtractor = (item: Transaksi, _index: number): string => item.id_transaksi.toString()
  _hapus = async (id: number, finisher: (() => void) | null = null) => {
  ...
  _getTerjual = async (finisher: (() => void) | null = null) => {
    let transaksi: any = await AsyncStorage.getItem('transaksi'),
...
export default withSafeAreaInsets<Omit<Props, 'insets'>>(Dashboard)
